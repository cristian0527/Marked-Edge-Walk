...
# __ MAIN __ 
const K          = 7 # Alabama
const EPSILON    = 0.025 # 0.01 before
...
function main(; initialization=nothing)
    ...
    county_ids = df[!, "COUNTYFP"]
    energy_fn = make_combined_energy(0.5, df[!, "COUNTYFP"], 0.0175, 4, 360) 
    # move betas and targets down 

    # 6 -> 5 -> 4 -> 5 -> 4? 
    # 0.25 -> 0.75 -> 1.5 -> 2 -> 1.75 -> maybe try 1.25
    # 400 -> 385 -> 375 -> 360 -> try 375 -> 380 -> 385
    # 0.015 -> 0.02 -> 0.0175
...
function prepare_warm_start()
    ...
    # seed_1 = JSON.parsefile("AL/AL_assignment_2_100000.json") # needed for seed n
    # louisiana_plans = JSON.parsefile("LA/LA_processed_precincts_Julia.json")

    con2026 = df[!, "CON"] # need this to start on CON
