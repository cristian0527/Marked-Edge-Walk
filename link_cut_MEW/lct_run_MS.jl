...
# __ MAIN __ 
const K          = 4 # Mississippi
const EPSILON    = 0.02 # 0.01 before
# within 5% balance of the population

const N_ITERS    = 250_000
...
function main(; initialization=nothing)
    ...
    county_ids = df[!, "COUNTYFP"]
    energy_fn = make_combined_energy(1, df[!, "COUNTYFP"], 0.010, 2, 220) 
    # sum_ntds_mega_mewing_sigma: 42 ouch
    # maybe too constraint? not moving much in the MEW


    # 0.75 -> 1.5 -> 1.75? -> 1
    # 225 -> 235 -> 220 -> maybe to just get the cut edges stable a bit more higher?
    # 0.015 -> 0.010
...
# 1 packed and 1 cracked in Alabama
# Seed run on Louisiana, not enacted -> 4 million 
