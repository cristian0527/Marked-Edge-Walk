...
* If you are working with an updated version of `networkx`, the following piece of code in the script, `json_graph.node_link_data(g)`, may not label the `links` data type in the `.json` as "`links`." Rather, `edges` will be the name of "`links`" data type key in the dictionary. We handled this by renaming the `edges` key name to be `links` since most of our code written in Julia used the `links` key name when we loaded in all of the states' data.
