...
compile_tmp_files(tmp_files, "warm_runs_percents_AL.jld2", "warm_runs_county_splits_AL.jld2", "warm_runs_cut_edges_AL.jld2", "warm_runs_black_percs_AL.jld2")
exit() 
...
county_array_15steps = load("warm_runs_county_splits_AL.jld2", "county_splits_arr")
plot(county_array_15steps)


cut_array_15steps = load("warm_runs_cut_edges_AL.jld2", "cut_edges_arr")
...
# p = plot(title="% dem over run AL", xlabel="step", ylabel="% dem", legend=true)
...
percs_array = load("warm_runs_percents_AL.jld2", "perc_arr")

enacted = [0.1741141, 0.26474234, 0.31798925, 0.32347805, 0.35225625,
       0.4280245, 0.59038907]
...
black_percs_array = load("warm_runs_black_percs_AL.jld2", "blackperc_arr")
enacted_black = [0.08832617, 0.18886573, 0.19869374, 0.21514372, 0.2614084,
       0.4124959, 0.52262181]


c = plot(title="% Black Box Plots AL")
...
p = plot(title="alabama dem % ", xlabel="run", ylabel="percs")

for i in 1:7
    list_data = vec(percs_array[i, :, 1])
    plot!(p, list_data, label="Series $i")
end
